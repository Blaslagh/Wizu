...
def chmuruj_slownik_ponad_x(wejscie_slownik=przetwarzanko.wczytywanko("Dane.txt"), sciezka='chmura.jpg', x = 2):
...
def chmury_4_all(sciezka):
	...
	for artysta in [x for x in os.listdir(sciezka) if os.path.isdir(sciezka+'\\'+x)]:
		...
		for rok in [x for x in os.listdir(sciezka+'\\'+artysta) if os.path.isdir(sciezka+'\\'+artysta+'\\'+x)]:
			try:
				chmuruj_slownik_ponad_x(przetwarzanko.wczytywanko(sciezka+'\\'+artysta+'\\'+rok+'.txt'),"Chmury\\"+artysta+'\\'+rok+'.jpg')
				print( "o", end = '' )
			except:
				print( "N", end = '' )
	return
